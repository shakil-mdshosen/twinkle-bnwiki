 ...
 ****************************************
 * Mode of invocation:     Tab ("ব্যাচ-সুরক্ষা")
...
Twinkle.batchprotect = function twinklebatchprotect() {
	if (Morebits.userIsSysop && ((mw.config.get('wgArticleId') > 0 && (mw.config.get('wgNamespaceNumber') === 2 ||
		...
		Twinkle.addPortletLink(Twinkle.batchprotect.callback, 'ব্যাচ সুরক্ষা', 'tw-pbatch', 'এই পাতায় সংযুক্ত পাতাগুলোর ব্যাচ সুরক্ষা');
	}
};
...
Twinkle.batchprotect.callback = function twinklebatchprotectCallback() {
	...
	Window.setTitle('ব্যাচ সুরক্ষা');
	Window.setScriptName('টুইংকল');
	Window.addFooterLink('সুরক্ষা নীতিমালা', 'WP:PROT');
	...
	form.append({
		...
		list: [
			{
				label: 'সুরক্ষা পরিবর্তন করুন',
				value: 'editmodify',
				name: 'editmodify',
				tooltip: 'শুধুমাত্র বিদ্যমান পাতার জন্য।',
				checked: true
			}
		]
	});
	...
	form.append({
		...
		list: [
			{
				label: 'স্থানান্তর সুরক্ষা পরিবর্তন করুন',
				value: 'movemodify',
				name: 'movemodify',
				tooltip: 'শুধুমাত্র বিদ্যমান পাতার জন্য।',
				checked: true
	...
	form.append({
		...
		list: [
			{
				label: 'তৈরি সুরক্ষা পরিবর্তন করুন',
				value: 'createmodify',
				name: 'createmodify',
				tooltip: 'শুধুমাত্র বিদ্যমান নয় এমন পাতার জন্য।',
				checked: true
			}
		]
	});
	...
	form.append({
		...
		label: 'কারণ:',
		size: 60,
		tooltip: 'সুরক্ষা লগ এবং পাতার ইতিহাসের জন্য।'
	});
...
Twinkle.batchprotect.callback.evaluate = function twinklebatchprotectCallbackEvaluate(event) {
	...
	if (numProtected > 0 && !confirm('আপনি ' + mw.language.convertNumber(numProtected) + ' টি পাতা সুরক্ষিত করতে যাচ্ছেন। আপনি কি নিশ্চিত?')) {
		return;
	}
	...
	if (!input.reason) {
		alert("আপনি খুবই কঠোর প্রশাসক! আপনাকে অব্যশই একটি কারণ উল্লেখ করতে হবে।");
		return;
	}
	...
	var batchOperation = new Morebits.batchOperation('সুরক্ষা সেটিংস প্রয়োগ করা হচ্ছে');
...
Twinkle.batchprotect.callbacks = {
	main: function(apiobj) {
		...
		var page = new Morebits.wiki.page(apiobj.params.page, 'সুরক্ষিত করা হচ্ছে ' + apiobj.params.page);
		...
		if (!takenAction) {
			Morebits.status.warn('সুরক্ষিত করা হচ্ছে ' + apiobj.params.page, 'page ' + (exists ? 'exists' : 'does not exist') + '; কিছুই করার নেই, এড়িয়ে যাওয়া হচ্ছে');
