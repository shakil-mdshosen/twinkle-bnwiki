...
Morebits.date.localeData = {
	...
	months: [msg('january', 'জানুয়ারি'), msg('february', 'ফেব্রুয়ারি'), msg('march', 'মার্চ'),
		msg('april', 'এপ্রিল'), msg('may_long', 'মে'), msg('june', 'জুন'),
		msg('july', 'জুলাই'), msg('august', 'আগস্ট'), msg('september', 'সেপ্টেম্বর'),
		msg('october', 'অক্টোবর'), msg('november', 'নভেম্বর'), msg('december', 'ডিসেম্বর')],
...
Morebits.date.prototype = {
	...
	monthHeaderRegex: function() {
		return new RegExp('^(==+)\\s*(?:' + this.getUTCMonthName() + '|' + this.getUTCMonthNameAbbrev() +
			')\\s+' + new Date().toLocaleDateString("bn", { year: "numeric" }) + '\\s*\\1', 'mg');
	},
	...
	monthHeader: function(level) {
		...
		var text = this.getUTCMonthName() + ' ' + new Date().toLocaleDateString("bn", { year: "numeric" });
...
Morebits.wiki.api.prototype = {
		...
		return $.ajax(ajaxparams).then(
			...
			function onAPIsuccess(response, statusText) {
				...
				if (this.onSuccess) {
					...
					this.statelem.info(msg('done', 'করা হয়েছে'));
				}
	...
	returnError: function(callerAjaxParameters) {
		if (this.errorCode === 'badtoken' && !this.badtokenRetry) {
			this.statelem.warn(msg('invalid-token-retrying', 'টোকেন অবৈধ। একটি নতুন টোকেন নেওয়া হচ্ছে ও পুনঃচেষ্টা করা হচ্ছে...'));
...
Morebits.wiki.page = function(pageName, status) {
	...
	if (!status) {
		status = msg('opening-page', pageName, '"' + pageName + '" খোলা হচ্ছে');
	}
	...
	this.load = function(onSuccess, onFailure) {
		...
		ctx.loadApi = new Morebits.wiki.api(msg('retrieving-page', 'পাতা খোঁজা হচ্ছে...'), ctx.loadQuery, fnLoadSuccess, ctx.statusElement, ctx.onLoadFailure);
	...
	this.save = function(onSuccess, onFailure) {
		...
		if (!ctx.pageLoaded && !canUseMwUserToken) {
			ctx.statusElement.error('অভ্যন্তরীণ ত্রুটি: লোড করা হয়নি এমন একটি পাতা সংরক্ষণ করার চেষ্টা করা হয়েছে!');
		...
		if (!ctx.editSummary) {
			...
			if (ctx.editMode === 'new' && ctx.newSectionTitle) {
				...
				ctx.statusElement.error('অভ্যন্তরীণ ত্রুটি: সংরক্ষণ করার আগে সারাংশ নির্ধারণ করা হয়নি!');
				ctx.onSaveFailure(this);
				return;
			}
		}
			...
			!confirm(
				ctx.fullyProtected === 'infinity'
					? msg('protected-indef-edit-warning', ctx.pageName,
						'আপনি সম্পূর্ণভাবে সুরক্ষিত একটি পাতায় সম্পাদনা করতে চলেছেন: "' + ctx.pageName + '" (অসীমভাবে সুরক্ষিত)।  \n\nসম্পাদনাটি করতে অগ্রসর হতে "ঠিক আছে" ক্লিক করুন অথবা এই সম্পাদনাটি না করতে "বাতিল" ক্লিক করুন।'
					)
					: msg('protected-edit-warning', ctx.pageName, ctx.fullyProtected,
						'আপনি সম্পূর্ণভাবে সুরক্ষিত একটি পাতায় সম্পাদনা করতে চলেছেন: "' + ctx.pageName +
					'" (সুরক্ষার মেয়াদ শেষ হবার তারিখ ' + new Morebits.date(ctx.fullyProtected).calendar('utc') + ' (ইউটিসি))।  \n\nসম্পাদনাটি করতে অগ্রসর হতে "ঠিক আছে" ক্লিক করুন অথবা এই সম্পাদনাটি না করতে "বাতিল" ক্লিক করুন।'
					)
			)
		) {
			ctx.statusElement.error(msg('protected-aborted', 'সম্পূর্ণ সুরক্ষিত পাতা সম্পাদনা করা বাতিল হয়েছে।'));
			ctx.onSaveFailure(this);
			return;
		}
		...
		ctx.saveApi = new Morebits.wiki.api(msg('saving-page', 'পাতা সংরক্ষণ করা হচ্ছে...'), query, fnSaveSuccess, ctx.statusElement, fnSaveError);
	...
	this.lookupCreation = function(onSuccess, onFailure) {
		...
		ctx.lookupCreationApi = new Morebits.wiki.api(msg('getting-creator', 'পাতা সৃষ্টির তথ্য সংগ্রহ করা হচ্ছে'), query, fnLookupCreationSuccess, ctx.statusElement, ctx.onLookupCreationFailure);
	...
	this.move = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('move')) {
			...
			ctx.moveApi = new Morebits.wiki.api(msg('getting-token', 'টোকেন সংগ্রহ করা হচ্ছে...'), query, fnProcessMove, ctx.statusElement, ctx.onMoveFailure);
	...
	this.patrol = function() {
		...
		if ($('.patrollink').length) {
			...
			ctx.patrolApi = new Morebits.wiki.api(msg('getting-token', 'টোকেন সংগ্রহ করা হচ্ছে...'), patrolQuery, fnProcessPatrol);
	...
	this.deletePage = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('delete')) {
			...
			ctx.deleteApi = new Morebits.wiki.api(msg('getting-token', 'টোকেন সংগ্রহ করা হচ্ছে...'), query, fnProcessDelete, ctx.statusElement, ctx.onDeleteFailure);
	...
	this.undeletePage = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('undelete')) {
			...
			ctx.undeleteApi = new Morebits.wiki.api(msg('getting-token', 'টোকেন সংগ্রহ করা হচ্ছে...'), query, fnProcessUndelete, ctx.statusElement, ctx.onUndeleteFailure);
	...
	this.protect = function(onSuccess, onFailure) {
		...
		ctx.protectApi = new Morebits.wiki.api(msg('getting-token', 'টোকেন সংগ্রহ করা হচ্ছে...'), query, fnProcessProtect, ctx.statusElement, ctx.onProtectFailure);
	...
	this.stabilize = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('stabilize')) {
			...
			ctx.stabilizeApi = new Morebits.wiki.api(msg('getting-token', 'টোকেন সংগ্রহ করা হচ্ছে...'), query, fnProcessStabilize, ctx.statusElement, ctx.onStabilizeFailure);
	...
	var fnCheckPageName = function(response, onFailure) {
		...
		if (page) {
			...
			if (response.redirects) {
				...
				new Morebits.status('Note', msg('redirected', ctx.pageName, resolvedName, ctx.pageName + ' থেকে ' + resolvedName + '-এ পুনর্নির্দেশিত হয়েছে'));
			}
			...
			ctx.statusElement.error(msg('redirect-resolution-fail', ctx.pageName, 'এর জন্য পুনঃনির্দেশগুলি সমাধান করা যায়নি: ' + ctx.pageName));
			onFailure(this);
	...
	var fnSaveSuccess = function() {
		...
		if (response.edit.result === 'Success') {
			...
			ctx.statusElement.info(['সম্পন্ন (', link, ')']);
		...
		if (response.edit.captcha) {
			ctx.statusElement.error('পাতাটি সংরক্ষণ করা যায়নি কারণ উইকির সার্ভার চাচ্ছে যে আপনি একটি ক্যাপচা পূরণ করুন।');
		} else {
			ctx.statusElement.error(msg('api-error-unknown', 'পাতা সংরক্ষণ করার সময় এপিআই থেকে অজানা ত্রুটি এসেছে'));
		}
	...
	var fnSaveError = function() {
		...
		if (errorCode === 'editconflict' && ctx.conflictRetries++ < ctx.maxConflictRetries) {
			...
			var purgeApi = new Morebits.wiki.api(msg('editconflict-purging', 'সম্পাদনা দ্বন্দ্ব শনাক্ত হয়েছে, সার্ভার ক্যাশ শোধন করা হচ্ছে'), purgeQuery, function() {
				--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds

				ctx.statusElement.info(msg('editconflict-retrying', 'সম্পাদনা দ্বন্দ্ব শনাক্ত হয়েছে, সম্পাদনা পুনঃপ্রয়োগ করা হচ্ছে'));
				if (fnCanUseMwUserToken('edit')) {
		...
		// check for network or server error
			...
			ctx.statusElement.info(msg('save-failed-retrying', 2, 'সংরক্ষণ ব্যর্থ হয়েছে, ২ সেকেন্ডের মধ্যে পুনরায় চেষ্টা করা হচ্ছে ...'));
			--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
		...
		// hard error, give up
			...
			switch (errorCode) {
				...
				case 'protectedpage':
					...
					ctx.statusElement.error('সম্পাদনা সংরক্ষণ ব্যর্থ হয়েছে: পাতাটি সুরক্ষিত');
					break;

				case 'abusefilter-disallowed':
					ctx.statusElement.error('সম্পাদনা ছাঁকনি এই সম্পাদনাটি করতে দেয়নি: "' + ctx.saveApi.getResponse().error.abusefilter.description + '"।');
					break;
				...
				default:
					ctx.statusElement.error('সম্পাদনা সংরক্ষণ ব্যর্থ হয়েছে: ' + ctx.saveApi.getErrorText());
			}
	...
	var fnLookupCreationSuccess = function() {
		...
		if (!rev) {
			ctx.statusElement.error(ctx.pageName + '-এর কোনও সংস্করণ খুঁজে পাওয়া যায়নি');
			ctx.onLookupCreationFailure(this);
			return;
		}
		...
		if (!ctx.lookupNonRedirectCreator || !isTextRedirect(rev.content)) {
			...
			if (!ctx.creator) {
				ctx.statusElement.error('পাতা সৃষ্টিকারীর নাম খুঁজে পাওয়া যায়নি');
				ctx.onLookupCreationFailure(this);
				return;
			}
			ctx.timestamp = rev.timestamp;
			if (!ctx.timestamp) {
				ctx.statusElement.error('পাতা তৈরির সময় তারিখ খুঁজে পাওয়া যায়নি');
				ctx.onLookupCreationFailure(this);
				return;
			}

			ctx.statusElement.info('পাতা সৃষ্টির তথ্য সংগ্রহ করা হচ্ছে');
			...
			ctx.lookupCreationApi = new Morebits.wiki.api('পাতা সৃষ্টির তথ্য সংগ্রহ করা হচ্ছে', ctx.lookupCreationApi.query, fnLookupNonRedirectCreator, ctx.statusElement, ctx.onLookupCreationFailure);
			ctx.lookupCreationApi.setParent(this);
			ctx.lookupCreationApi.post();
		}
	...
	var fnPreflightChecks = function(action, onFailure) {
		...
		if (!Morebits.userIsSysop && action !== 'move') {
			ctx.statusElement.error('পাতা ' + action + ' করা যায়নি : শুধুমাত্র প্রশাসক তা করতে পারবে');
			onFailure(this);
			return false;
		}
	...
	var fnProcessChecks = function(action, onFailure, response) {
		...
		if (!response.tokens.csrftoken) {
			ctx.statusElement.error('টোকেন সংগ্রহ ব্যর্থ হয়েছে।');
			onFailure(this);
			return false;
		}
		return true; // all OK
	...
	var fnProcessMove = function() {
		...
		ctx.moveProcessApi = new Morebits.wiki.api(msg('moving-page', 'পাতা স্থানান্তর করা হচ্ছে...'), query, ctx.onMoveSuccess, ctx.statusElement, ctx.onMoveFailure);
	...
	var fnProcessPatrol = function() {
		...
		var patrolStat = new Morebits.status('টহলকৃত হিসেবে পাতা চিহ্নিত করা হচ্ছে');

		ctx.patrolProcessApi = new Morebits.wiki.api('টহলদান করা হচ্ছে...', query, null, patrolStat);
	...
	var fnProcessDelete = function() {
		...
		ctx.deleteProcessApi = new Morebits.wiki.api('পাতা অপসারণ করা হচ্ছে...', query, ctx.onDeleteSuccess, ctx.statusElement, fnProcessDeleteError);
	...
	var fnProcessDeleteError = function() {
		...
		if (errorCode === 'internal_api_error_DBQueryError' && ctx.retries++ < ctx.maxRetries) {
			...
			ctx.statusElement.error('পাতা অপসারণ করা যায়নি, কারণ এটি আর বিদ্যমান নেই');
	...
	var fnProcessUndelete = function() {
		...
		ctx.undeleteProcessApi = new Morebits.wiki.api('পাতা পুনরুদ্ধার করা হচ্ছে...', query, ctx.onUndeleteSuccess, ctx.statusElement, fnProcessUndeleteError);
	...
	var fnProcessUndeleteError = function() {
		...
		// hard error, give up
			...
			ctx.statusElement.error('পাতাটি পুনরুদ্ধার করা ব্যর্থ হয়েছে: ' + ctx.undeleteProcessApi.getErrorText());
			if (ctx.onUndeleteFailure) {
				ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi);  // invoke callback
			}
		}
	...
	var fnProcessProtect = function() {
		...
		ctx.protectProcessApi = new Morebits.wiki.api('পাতা সুরক্ষিত করা হচ্ছে...', query, ctx.onProtectSuccess, ctx.statusElement, ctx.onProtectFailure);
	...
	var fnProcessStabilize = function() {
		...
		*/
		...
		ctx.stabilizeProcessApi = new Morebits.wiki.api('স্থিতিশীলতা সেটিং কনফিগার করা হচ্ছে...', query, ctx.onStabilizeSuccess, ctx.statusElement, ctx.onStabilizeFailure);
...
Morebits.wiki.preview = function(previewbox) {
	...
	this.beginRender = function(wikitext, pageTitle, sectionTitle) {
		...
		var renderApi = new Morebits.wiki.api('লোড করা হচ্ছে...', query, fnRenderSuccess, new Morebits.status('প্রাকদর্শন'));
	...
	var fnRenderSuccess = function(apiobj) {
		...
		if (!html) {
			apiobj.statelem.error('প্রাকদর্শন ব্যর্থ হয়েছে, অথবা টেমপ্লেটটি ফাঁকা');
			return;
		}
...
Morebits.userspaceLogger = function(logPageName) {
	...
	this.log = function(logText, summaryText) {
		...
		var page = new Morebits.wiki.page('ব্যবহারকারী:' + mw.config.get('wgUserName') + '/' + logPageName,
			'ব্যবহারকারী নামস্থানের লগে ভুক্তি যোগ করা হয়েছে'); // make this '... to ' + logPageName ?
...
Morebits.batchOperation = function(currentAction) {
		...
		if (arg instanceof Morebits.wiki.api || arg instanceof Morebits.wiki.page) {
			...
			if (ctx.options.preserveIndividualStatusLines) {
				if (arg.getPageName || arg.pageName || (arg.query && arg.query.title)) {
					...
					statelem.info(msg('batch-done-page', pageName, 'সম্পন্ন ([[' + pageName + ']])'));
				} else {
					// we don't know the page title - just display a generic message
					statelem.info(msg('done', 'করা হয়েছে'));
				}
			} else {
			...
			new Morebits.status(arg, msg('batch-done-page', arg, 'সম্পন্ন ([[' + arg + ']])'));
		}
	...
	var fnDoneOne = function() {
		...
		if (ctx.countFinished < total) {
			...
			var statusString = msg('batch-progress', ctx.countFinishedSuccess, ctx.countFinished, 'করা হয়েছে (' + ctx.countFinishedSuccess +
...
Morebits.simpleWindow.prototype = {
	...
	addContent: function(content) {
		...
		$(this.content).find('input[type="submit"], button[type="submit"]').each(function(key, value) {
			...
			button.textContent = value.hasAttribute('value') ? value.getAttribute('value') : value.textContent ? value.textContent : msg('submit', 'জমা দিন');
